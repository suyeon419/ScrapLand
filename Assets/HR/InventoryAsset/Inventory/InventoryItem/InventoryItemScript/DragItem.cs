...
namespace InventorySystem
{
    ...
    internal class DragItem : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler
    {
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            if (item == null)
            {
                Debug.LogWarning("[DragItem] OnBeginDrag: item is null!");
                return;
            }
...
/*                // 드롭된 슬롯이 핫바면 선택 및 손에 들기 상태 갱신
        ...
        private void HandleInvalidPlacement(bool isOverride = false)
        {
            if (item == null)
            {
                Debug.LogWarning("[DragItem] item is null! Cannot handle invalid placement.");
                return;
            }

            if (CurrentSlot == null)
            {
                Debug.LogWarning("[DragItem] CurrentSlot is null! Cannot return item.");
                return;
            }

            if (InventoryController.instance == null)
            {
                Debug.LogError("[DragItem] InventoryController.instance is null!");
                return;
            }
